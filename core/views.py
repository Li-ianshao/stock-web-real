...
from core.utils.fetcher import download_stock_data, fetch_stock_data, load_or_fetch_stock_data, clear_all_pickles, fetch_historical_data, getData, get_sp500_tickers, get_latest_stock_price
...
def stock_api(request, symbol, data_type):

    if data_type == 'hist':
        #這是只需要歷史股票資料用來呈現現價與技術面的
        stock_data = download_stock_data([symbol], period='3y')

        df = stock_data[symbol]["history"]
        ...
        # 假設 hist 是從 yf.download 抓下來的單檔股票 DataFrame (包含 actions=True)
        hist = stock_data[symbol]["history"]

        # 1. 取得股利欄位
        if "Dividends" in hist.columns:
            div_data = hist["Dividends"]

            # 若為 DataFrame (MultiIndex)，轉為單一 Series
            if isinstance(div_data, pd.DataFrame):
                div_series = div_data.squeeze()
            else:
                div_series = div_data

            # 2. 篩選大於 0 且非 NaN 的除息紀錄
            valid_divs = div_series[
                (div_series > 0) & (~div_series.isna())
            ]
            ...
            dividends_list = [
                ...
                for dt, val in valid_divs.items()
            ]
        else:
            dividends_list = []
        ...
        return JsonResponse(
            {
                "error": f"Invalid data_type '{data_type}'. Must be 'hist' or 'info'."
            },
            status=400,
        )
